...
const noiseGen = (
	...
	scale: number = 4,
	scaleFactor = 4,
	octaveAmount: number = 3,
	seed = '',
	margin = 0,
	shapeFactor1 = 1,
	shapeFactor2 = 1
) => {
	const octaves = new Array(octaveAmount);
	const prng = alea(seed);
	const noise = createNoise2D(prng);
	const noiseSummed = createMatrix(width, height);

	// noisemaps for each octave
	for (let i = 0; i < octaveAmount; i++) {
		let sclfactor = scaleFactor;
		if (i > 0) sclfactor = scaleFactor * i;
		octaves.push(calculateNoise(noise, scale * sclfactor, width, height));
	}
	...
	// TODO: figure out how to sum all from array
	const calculateSum = () => {
		for (let i = 0; i < octaveAmount; i++) {}
	};
};
